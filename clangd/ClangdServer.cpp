...
ClangdServer::findDefinitions(PathRef File, Position Pos) {
  ...
  Resources->getAST().get()->runUnderLock([Pos, &Result, this](ParsedAST *AST) {
    if (!AST)
      return;
    Result = clangd::findDefinitions(*AST, Pos, *IndexDataProvider, Logger);
  });
  return make_tagged(std::move(Result), TaggedFS.Tag);
}
...
llvm::Expected<Tagged<std::vector<Location>>>
ClangdServer::findReferences(PathRef File, Position Pos, bool IncludeDeclaration) {
  assert(Indexer);
  auto FileContents = DraftMgr.getDraft(File);
  assert(FileContents.Draft &&
  ...
  std::lock_guard<std::recursive_mutex> Lock(IndexMutex);
  std::vector<Location> Result;
  Resources->getAST().get()->runUnderLock([Pos, &Result, IncludeDeclaration, this](ParsedAST *AST) {
    if (!AST)
      return;
    Result = clangd::findReferences(*AST, Pos, IncludeDeclaration, *IndexDataProvider, Logger);
  });
